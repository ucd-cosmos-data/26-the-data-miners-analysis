...
## Project location and raw-data boundary

The project is intentionally located inside the main dataset directory, as requested:

```text
BCI Database/
├── Instructions/               # publisher data
├── OV_Scenarios/               # publisher data
├── Questionnaires/             # publisher data
├── Signals/                    # publisher data
├── Perfomances.csv             # publisher data
├── Perfomances.xlsx            # publisher data
└── bci_cleaning/
    ├── raw -> ..
    ├── notebooks/
    │   ├── 01_inventory.ipynb
    ...
    │   ├── 04_clean.ipynb
    │   └── 05_verify.ipynb
    ├── cleaned/                # APFS copy-on-write output; git-ignored
    ├── reports/
    ├── logs/
    ├── support/bci_core.py
    ├── tests/
...
`raw/` is a relative symbolic link to the enclosing `BCI Database/`. Every raw traversal explicitly prunes the complete `bci_cleaning/` subtree, so notebooks, reports, logs, tests, and cleaned output cannot enter the dataset inventory or clone. The APFS clone likewise skips the top-level `bci_cleaning/` directory before writing `bci_cleaning/cleaned/`.

Adding this project changes only the directory membership of `BCI Database/`; no publisher-supplied file is renamed, moved, overwritten, or deleted. The hash-backed raw corpus is the publisher content outside `bci_cleaning/`.
...
Activate the environment above, stay in `bci_cleaning/`, and execute the notebooks in order:
...
python -m jupyter nbconvert --to notebook --execute --inplace notebooks/04_clean.ipynb
python -m jupyter nbconvert --to notebook --execute --inplace notebooks/05_verify.ipynb
```

The inventory reads only the Zenodo ZIP central directory through HTTP byte ranges and compares every local publisher path, size, and CRC32 without downloading the 27.5 GB archive. SHA-256 progress is checkpointed in `logs/inventory_hash_cache.jsonl`.

The first three notebooks are independently runnable and read-only. `04_clean.ipynb` refuses to run when archive verification is absent or failed, a fatal/error validation issue remains, the raw SHA-256 snapshot changed, fewer than 10 GiB are free, `raw/` is not the expected symlink, or `cleaned/` is nonempty. To rebuild, first retain or remove the existing derived `cleaned/` tree deliberately; never remove anything from raw.
...
3. Administrative artifacts
   ...
   - Omit `.DS_Store` files and invalid `~$` Office owner-lock files from `cleaned/`.
...
- `dataset_structure.md`: type counts and the complete publisher-data tree, excluding `bci_cleaning/`.
...
- `cleaning_manifest.csv`: one-to-one raw/cleaned lineage and action record.
- `cleaning_report.md`: human-readable cleaning summary.
- `post_clean_validation.csv`: independent raw and cleaned hash/equivalence results.
- `pipeline.jsonl`: timestamped audit events without questionnaire free text or medication responses.
...
- Final verification: all 1,073 raw hashes unchanged, all 1,068 cleaned files traceable, all 694 GDF hashes identical, and all participant IDs preserved.
- Final status: `PASS`, with the A40 R3 and C83 warnings retained in `validation_issues.csv`.

The cleaned dataset is complete only when `post_clean_summary.md` reports `PASS`.
